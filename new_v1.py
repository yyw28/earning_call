...
def perform_diarization(audio_path,company_dir):
    ...
    trimmed_audio_path = os.path.join(company_dir,"trimmed_audio.wav")
    ...
    return diarization_result,trimmed_audio_path

import whisper
from pydub import AudioSegment

def transcribe_audio_segment(company_dir,args):
    ...
    speaker_folder = os.path.join(company_dir, speaker_id)
    os.makedirs(speaker_folder, exist_ok=True)
    
    # Save the segment as a temporary file
    temp_audio_filename = f"temp_segment_{start_time:.2f}.wav"
    temp_audio_path = os.path.join(speaker_folder,temp_audio_filename)
...
def transcribe_segments_parallel(company_dir,audio_path, diarization_result):
    ...
    with Pool(num_workers) as pool:
        transcribed_segments = pool.map(transcribe_audio_segment,[(company_dir,args) for arg in args])
...
def main():
    ...
    for video in video_files:
        ...
        diarization_result,trimmed_audio_path = perform_diarization(audio_path,company_dir)
